# ngNailArt - TEST AJANI
# ------------------------------------------------------------
# Guncellemeden ONCE bir kez, SONRA bir kez calistir. Her calisma:
#   1) Derleme (ng build)  -> web'de hata cikar mi? (en kritik)
#   2) Birim testler (vitest)
#   3) Lint (istege bagli: -Lint)
# Sonuc test-reports klasorune yazilir ve BIR ONCEKI calisma ile
# karsilastirilir. Boylece "guncelleme bir seyi bozdu mu" aninda gorunur.
#
# Kullanim:  test.bat (cift tik)   |   Lint de calissin: test.bat -Lint
...
$pass = 0; $fail = 0
if ($unit.Out -match '(\d+)\s+passed') { $pass = [int]$Matches[1] }
if ($unit.Out -match '(\d+)\s+failed') { $fail = [int]$Matches[1] }
...
# Onceki raporla karsilastir
$prev = Get-ChildItem $reportsDir -Filter 'rapor-*.txt' -ErrorAction SilentlyContinue |
...
if ($prev) {
    ...
    if ($pb -eq 'FAIL' -and $nb -eq 'OK') { $changes += '+++ Derleme DUZELDI (onceki FAIL -> simdi OK).' }
    ...
    if ($pp -ge 0 -and $pass -lt $pp) { $changes += "!!! Test regresyonu: onceki $pp gecti, simdi $pass ($($pp - $pass) test bozuldu)." }
    if ($pp -ge 0 -and $pass -gt $pp) { $changes += "+++ $($pass - $pp) yeni test gecti." }
    if ($changes.Count -eq 0) { $changes += 'Degisiklik yok. Guvenli.' }
} else {
    $changes += 'Ilk calisma - karsilastiracak onceki rapor yok (baslangic/baseline kaydedildi).'
}

# Rapor dosyasini yaz
$lines = @()
$lines += "=== ngNailArt TEST RAPORU ==="
$lines += "Tarih: $human"
$lines += "Git  : $git"
$lines += $statusLine
$lines += ""
...
$lines += ("Lint (ng lint)         : $lintStatus")
$lines += ""
$lines += "--- ONCEKI CALISMAYA GORE DEGISIM ---"
if ($prevStatus) { $lines += "Onceki: $prevStatus"; $lines += "Simdi : $statusLine" }
$lines += $changes
if (-not $buildOk) {
    $lines += ""; $lines += "--- DERLEME HATA CIKTISI (son satirlar) ---"
    $lines += (($build.Out -split "`n") | Select-Object -Last 30)
}
if (-not $unitOk) {
    $lines += ""; $lines += "--- BIRIM TEST CIKTISI (son satirlar) ---"
    $lines += (($unit.Out -split "`n") | Select-Object -Last 25)
}
$lines | Set-Content -Encoding UTF8 $reportPath

# Ekran ozeti
Write-Host ""
Write-Host "=============== OZET ===============" -ForegroundColor Magenta
...
foreach ($c in $changes) {
    $col = if ($c -like '!!!*') { 'Red' } elseif ($c -like '+++*') { 'Green' } else { 'Gray' }
    Write-Host "  $c" -ForegroundColor $col
}
Write-Host ""
Write-Host "Rapor: $reportPath" -ForegroundColor Gray
Write-Host "====================================" -ForegroundColor Magenta
Write-Host "Kapatmak icin bir tusa bas..." -ForegroundColor DarkGray
