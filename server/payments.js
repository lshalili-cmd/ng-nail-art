...
/** Türkçe/ASCII-dışı karakterleri ASCII'ye çevirir. iyzico'nun PKI imzası bunlarda
 *  "Geçersiz imza" verebildiği için sepet/alıcı metinleri sadeleştirilir. */
function toAscii(s) {
...
async function iyzicoCheckout({ itemName, amount, currency, userId, buyer, baseUrl, ref }) {
  ...
  const bi = buyer || {};
  const name = toAscii(bi.name) || 'Musteri';
  const surname = toAscii(bi.surname) || 'NailArt';
  const email = String(bi.email == null ? '' : bi.email).trim() || 'musteri@miraclenailart.com';
  const gsm = String(bi.phone == null ? '' : bi.phone).trim() || '+905000000000';
  const contact = `${name} ${surname}`.trim();
  const product = toAscii(itemName) || 'Uyelik';

  console.log(`💳 iyzico(SDK) istek → fiyat: ${price} ${currency || 'TRY'} · ürün: ${product}`);
  ...
  const request = {
    ...
    basketItems: [{ id: 'item1', name: product, category1: 'Membership', itemType: 'VIRTUAL', price }],
  };
