...
### Kayıt akışı (OTP + şifre kuralı + tekillik)

Kayıt zorunlu alanlar: **ad, soyad, e-posta, telefon**. E-posta **ve** telefon tekildir (çoklu
üyelik engeli). Şifre kuralı: **tam 1 harf (büyük/küçük farketmez) + geri kalanı rakam, en az 6
karakter** (örn. `a12345`). Yeni kayıtta telefona **OTP** gönderilir; kod doğrulanınca hesap
aktifleşir ve token verilir. `POST /api/auth/verify-otp`, `/resend-otp`, `/forgot`, `/reset` uçları eklendi.

`User` tablosu değiştiği için **veritabanını yenilemek** gerekir (dev DB — verileri sıfırlar, örnekleri
yeniden yükler; eski test kullanıcısı da silinir):
...
### SMS (OTP) ve E-posta (şifre sıfırlama)

Sağlayıcı yoksa **DEMO** modunda çalışır: OTP kodu ve sıfırlama bağlantısı API yanıtında döner ve
sunucu konsoluna yazılır — gerçek SMS/e-posta gitmeden akış test edilebilir. Gerçeğe geçmek için
`.env`'e anahtar ekleyin (yalnızca kullanacağınızı) ve gerekli paketi kurun:

```
# SMS — Twilio (npm i twilio) VEYA Netgsm (paket gerekmez)
TWILIO_ACCOUNT_SID=...   TWILIO_AUTH_TOKEN=...   TWILIO_FROM=+1...
NETGSM_USER=...          NETGSM_PASS=...         NETGSM_HEADER=BASLIK
# E-posta — SMTP (npm i nodemailer)
SMTP_HOST=smtp.example.com  SMTP_PORT=587  SMTP_USER=...  SMTP_PASS=...  SMTP_FROM="Miracle <no-reply@...>"
```
